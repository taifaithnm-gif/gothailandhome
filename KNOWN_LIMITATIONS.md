...
2. **Developer logos** — All 20 Developer Master logos remain **PLACEHOLDER**. Developer Center uses a neutral mark; placeholders are not treated as trademarks.
...
5. **Marketplace forms** — Shared Alpha form kit + hub entry layer (`/[lang]/marketplace`). Lead Foundation M4 adds shared `/leads/success` and `/leads/error` (noindex). Still not a completed partnership/ops CRM workflow (no email automation).
