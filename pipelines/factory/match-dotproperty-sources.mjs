#!/usr/bin/env node
/**
 * Soft-match DotProperty ↔ PropertyHub and LivingInsider.
...
const dp = loadListings("listings-dotproperty.json", "dotproperty");
...
const report = {
  ...
  dotproperty_listings: dp.length,
...
for (const c of candidates) {
  const { data: a } = await admin
    .from("properties")
    .select("id")
    .eq("external_ref", c.dotproperty.external_ref)
  ...
  const { error } = await admin.from("listing_duplicate_candidates").insert({
    property_id_a: idA,
    property_id_b: idB,
    match_reason: `cross_source_soft_match_dotproperty_${c.against}`,
    confidence: 0.55,
    evidence: c,
    status: "open",
  });
