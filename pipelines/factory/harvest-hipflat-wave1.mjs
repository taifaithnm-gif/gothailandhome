#!/usr/bin/env node
/**
 * Hipflat Wave 1 harvest for mapped Bangkok projects.
 * Does NOT modify PropertyHub, LivingInsider, or DDproperty packages.
 * Does NOT attempt to bypass Cloudflare.
 *
 * Usage:
 *   node pipelines/factory/harvest-hipflat-wave1.mjs [--per-project=N] [--limit-projects=N] [--only=slug] [--dry-run]
 */
...
const map = JSON.parse(
  readFileSync(
    join(ROOT, "pipelines/factory/hipflat/project-map.wave1.json"),
    "utf8",
  ),
);
...
const evidenceDir = join(ROOT, "pipelines/factory/hipflat/_runs");
...
for (const p of projects) {
  process.stderr.write(`HF harvest ${p.project_slug}...\n`);
  const idMap = new Map();
  ...
  const outPkg = {
    ...
    notes:
      "Hipflat harvest — verified only when public detail exposes priced offer + listing type. No fabricated fields. No Cloudflare bypass. PropertyHub/LivingInsider/DDproperty untouched.",
    listings,
  };
...
if (summary.totals.validated === 0 && summary.totals.search_pages_blocked > 0) {
  ...
  summary.status_reason =
    "Hipflat search/detail pages returned Cloudflare challenges; no priced listings could be validated without fabricating data or bypassing Cloudflare.";
