#!/usr/bin/env node
/**
 * FazWaz-only importer.
 * - Reads listings-fazwaz.json only
 * - Resolves existing project by slug (no parent rewrites)
 * - Upserts ONLY rows with source=fazwaz
 * - Never updates prior sources properties
 */
import {
  existsSync,
  mkdirSync,
...
const OUT = join(ROOT, "pipelines/factory/fazwaz/_runs");
...
const projects = readdirSync(join(ROOT, "content/projects"))
  .filter((d) =>
    existsSync(join(ROOT, "content/projects", d, "listings-fazwaz.json")),
  )
  .filter((d) => {
    const n = (
      JSON.parse(
        readFileSync(
          join(ROOT, "content/projects", d, "listings-fazwaz.json"),
          "utf8",
        ),
      ).listings || []
    ).length;
...
if (!projects.length) {
  ...
  report.note =
    "No listings-fazwaz.json packages with listings — nothing to import (harvest empty/blocked).";
  writeFileSync(join(OUT, "import-fz-only.json"), JSON.stringify(report, null, 2));
...
for (const slug of projects) {
  const pkg = JSON.parse(
    readFileSync(
      join(ROOT, "content/projects", slug, "listings-fazwaz.json"),
      "utf8",
    ),
  );
  ...
  for (const listing of pkg.listings || []) {
    if (listing.source && listing.source !== "fazwaz") {
      report.skipped += 1;
      continue;
    }
    ...
    if (existing?.source && existing.source !== "fazwaz") {
      report.errors.push({
        slug,
        external_ref: listing.external_ref,
        error: "refused_non_fazwaz_row",
      });
      continue;
    }
    ...
    if (propertyId && (prev == null || prev !== next)) {
      await admin.from("listing_price_history").insert({
        ...
        listing_url: listing.listing_url,
        observed_at: now,
        note: prev == null ? "fazwaz_import" : "price_change",
      });
