#!/usr/bin/env node
/**
 * LivingInsider-only importer.
 * - Reads listings-livinginsider.json only
 * - Resolves existing project/location/district by slug (no parent rewrites)
 * - Upserts ONLY rows with source=livinginsider
 * - Never updates PropertyHub properties
 *
 * Usage: node pipelines/factory/livinginsider/import-li-only.mjs [--dry-run] [--limit=N]
 */
import {
  existsSync,
  mkdirSync,
...
const projects = readdirSync(join(ROOT, "content/projects"))
  .filter((d) =>
    existsSync(join(ROOT, "content/projects", d, "listings-livinginsider.json")),
  )
  .filter((d) => {
    const n = (
      JSON.parse(
        readFileSync(
          join(ROOT, "content/projects", d, "listings-livinginsider.json"),
          "utf8",
        ),
      ).listings || []
    ).length;
...
for (const slug of projects) {
  const pkg = JSON.parse(
    readFileSync(
      join(ROOT, "content/projects", slug, "listings-livinginsider.json"),
      "utf8",
    ),
  );
  ...
  const locSlug = manifest.location?.slug;
  let locationId = project.location_id;
  if (!locationId && locSlug) {
  ...
  if (!locationId) {
    report.errors.push({ slug, error: "missing_location_id" });
    continue;
  }
  ...
  for (const listing of pkg.listings || []) {
    if (listing.source && listing.source !== "livinginsider") {
      report.skipped += 1;
      continue;
    }
    ...
    // ONLY match existing LivingInsider rows — never PropertyHub
    const { data: existing } = await admin
      .from("properties")
      .select("id,price_thb,published_at,source")
      .eq("source", "livinginsider")
      .eq("external_ref", listing.external_ref)
      .maybeSingle();

    if (existing?.source && existing.source !== "livinginsider") {
      report.errors.push({
        slug,
        external_ref: listing.external_ref,
        error: "refused_non_livinginsider_row",
      });
      continue;
    }
    ...
    if (propertyId && (prev == null || prev !== next)) {
      await admin.from("listing_price_history").insert({
        ...
        listing_url: listing.listing_url,
        observed_at: now,
        note: prev == null ? "livinginsider_import" : "price_change",
      });
    ...
    await admin.from("listing_verification_events").insert({
      ...
      source: "livinginsider",
      source_listing_id: identity.source_listing_id,
      event_type: "livinginsider_import",
      to_status: verificationStatus,
      evidence: { external_ref: listing.external_ref, project_slug: slug },
      batch_key: `livinginsider-wave1-${now.slice(0, 10)}`,
    });
    report.events += 1;
  }
  ...
  process.stderr.write(
    `LI import ${slug}: +${projectInserted} ~${projectUpdated}\n`,
  );
}

report.finished_at = new Date().toISOString();
writeFileSync(join(OUT, "import-li-only.json"), JSON.stringify(report, null, 2));
