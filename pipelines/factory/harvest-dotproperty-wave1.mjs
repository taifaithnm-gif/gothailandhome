#!/usr/bin/env node
/**
 * DotProperty Wave 1 harvest for mapped Bangkok projects.
 * Does NOT modify PropertyHub, LivingInsider, DDproperty, or Hipflat packages.
 *
 * Usage:
 *   node pipelines/factory/harvest-dotproperty-wave1.mjs [--per-project=N] [--limit-projects=N] [--only=slug] [--dry-run]
 */
...
const map = JSON.parse(
  readFileSync(
    join(ROOT, "pipelines/factory/dotproperty/project-map.wave1.json"),
    "utf8",
  ),
);
const projects = Object.values(map.projects || {})
  .filter((p) => p.project_id && p.search_sale_url)
  .filter((p) => !onlySet || onlySet.has(p.project_slug))
  .slice(0, limitProjects);

const evidenceDir = join(ROOT, "pipelines/factory/dotproperty/_runs");
...
for (const p of projects) {
  process.stderr.write(`DP harvest ${p.project_slug} (id=${p.project_id})...\n`);
  const idMap = new Map();
  ...
  const outPkg = {
    ...
    notes:
      "DotProperty harvest — verified only when public RealEstateListing ld+json exposes priced offer + listing type. No fabricated fields. Prior sources untouched.",
    listings,
  };
...
if (summary.totals.validated === 0 && summary.totals.search_pages_blocked > 0) {
  ...
  summary.status_reason =
    "DotProperty pages returned Cloudflare/HTTP failures; no priced listings validated.";
