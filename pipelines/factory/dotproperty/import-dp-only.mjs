#!/usr/bin/env node
/**
 * DotProperty-only importer.
 * - Reads listings-dotproperty.json only
 * - Resolves existing project by slug (no parent rewrites)
 * - Upserts ONLY rows with source=dotproperty
 * - Never updates PropertyHub, LivingInsider, DDproperty, or Hipflat properties
 */
import {
  existsSync,
  mkdirSync,
...
const OUT = join(ROOT, "pipelines/factory/dotproperty/_runs");
...
await admin.from("listing_source_priority").upsert(
  {
    source: "dotproperty",
    priority: 24,
    notes: "DotProperty Wave 1 adapter",
  },
  { onConflict: "source" },
);
...
const projects = readdirSync(join(ROOT, "content/projects"))
  .filter((d) =>
    existsSync(join(ROOT, "content/projects", d, "listings-dotproperty.json")),
  )
  .filter((d) => {
    const n = (
      JSON.parse(
        readFileSync(
          join(ROOT, "content/projects", d, "listings-dotproperty.json"),
          "utf8",
        ),
      ).listings || []
    ).length;
...
if (!projects.length) {
  ...
  report.note =
    "No listings-dotproperty.json packages with listings — nothing to import (harvest empty/blocked).";
  writeFileSync(join(OUT, "import-dp-only.json"), JSON.stringify(report, null, 2));
...
for (const slug of projects) {
  const pkg = JSON.parse(
    readFileSync(
      join(ROOT, "content/projects", slug, "listings-dotproperty.json"),
      "utf8",
    ),
  );
  ...
  for (const listing of pkg.listings || []) {
    if (listing.source && listing.source !== "dotproperty") {
      report.skipped += 1;
      continue;
    }
    ...
    if (existing?.source && existing.source !== "dotproperty") {
      report.errors.push({
        slug,
        external_ref: listing.external_ref,
        error: "refused_non_dotproperty_row",
      });
      continue;
    }
    ...
    if (propertyId && (prev == null || prev !== next)) {
      await admin.from("listing_price_history").insert({
        ...
        listing_url: listing.listing_url,
        observed_at: now,
        note: prev == null ? "dotproperty_import" : "price_change",
      });
    ...
    await admin.from("listing_verification_events").insert({
      ...
      source: "dotproperty",
      source_listing_id: identity.source_listing_id,
      event_type: "dotproperty_import",
      to_status: verificationStatus,
      evidence: { external_ref: listing.external_ref, project_slug: slug },
      batch_key: `dotproperty-wave1-${now.slice(0, 10)}`,
    });
    report.events += 1;
  }
