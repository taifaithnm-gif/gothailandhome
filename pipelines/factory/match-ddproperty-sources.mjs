#!/usr/bin/env node
/**
 * Soft-match DDproperty ↔ PropertyHub and LivingInsider.
...
const dd = loadListings("listings-ddproperty.json", "ddproperty");
...
const candidates = [];
for (const d of dd) {
  for (const phHit of phBySoft.get(d.soft) || []) {
    candidates.push({
      ...
      soft_match_fingerprint: d.soft,
      note: "candidate only — never auto-merge",
    });
  }
  for (const liHit of liBySoft.get(d.soft) || []) {
    candidates.push({
      match_reason: "cross_source_soft_match",
...
for (const c of candidates) {
  const { data: a } = await admin
    .from("properties")
    .select("id")
    .eq("external_ref", c.ddproperty.external_ref)
  ...
  const { error } = await admin.from("listing_duplicate_candidates").insert({
    property_id_a: idA,
    property_id_b: idB,
    match_reason: `cross_source_soft_match_ddproperty_${c.against}`,
    confidence: 0.55,
    evidence: c,
    status: "open",
  });
