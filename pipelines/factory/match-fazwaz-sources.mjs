#!/usr/bin/env node
/**
 * Soft-match FazWaz ↔ PropertyHub, LivingInsider, DotProperty.
...
const fz = loadListings("listings-fazwaz.json", "fazwaz");
const ph = loadListings("listings.json", "propertyhub");
const li = loadListings("listings-livinginsider.json", "livinginsider");
const dprop = loadListings("listings-dotproperty.json", "dotproperty");
...
const dpBySoft = indexBySoft(dprop);
...
for (const h of fz) {
  ...
  for (const dpHit of dpBySoft.get(h.soft) || []) {
    candidates.push({
      match_reason: "cross_source_soft_match",
      against: "dotproperty",
      confidence: 0.55,
      ...
      other: {
        source: "dotproperty",
        external_ref: dpHit.external_ref,
        source_listing_id: dpHit.source_listing_id,
        listing_url: dpHit.listing_url,
        price_thb: dpHit.price_thb,
        project_slug: dpHit.project_slug,
      },
      soft_match_fingerprint: h.soft,
      note: "candidate only — never auto-merge",
    });
...
for (const c of candidates) {
  const { data: a } = await admin
    .from("properties")
    .select("id")
    .eq("external_ref", c.fazwaz.external_ref)
    .eq("source", "fazwaz")
    .maybeSingle();
  ...
  const { error } = await admin.from("listing_duplicate_candidates").insert({
    property_id_a: idA,
    property_id_b: idB,
    match_reason: `cross_source_soft_match_fazwaz_${c.against}`,
    confidence: 0.55,
    evidence: c,
    status: "open",
  });
