/**
 * DDproperty adapter — crawl / parse / normalize.
 * Does not touch PropertyHub or LivingInsider modules.
 *
 * Note: www.ddproperty.com is Cloudflare-protected from many automated
 * environments; harvest records blockers and never fabricates listings.
 */
import { deriveListingIdentity } from "../lib/listing-identity.mjs";

export const SOURCE = "ddproperty";
export const UA =
...
/** Extract listing ids from DDproperty HTML / JSON blobs. */
export function extractListingRefs(html) {
  const out = new Map(); // id -> path
  const patterns = [
    /href="(\/en\/[^"#?\s]*?(?:property|listing|condo)[^"#?\s]*?-(\d{6,}))"/gi,
    /href="(https:\/\/www\.ddproperty\.com\/en\/[^"#?\s]+?-(\d{6,}))"/gi,
    /"listingId"\s*:\s*"?(\d{6,})"?/gi,
    /"id"\s*:\s*(\d{6,})/gi,
  ];
  for (const re of patterns) {
    ...
    while ((m = re.exec(html))) {
      const id = m[2] || m[1];
      const path = m[2] ? m[1] : null;
      if (/^\d{6,}$/.test(String(id))) {
        if (!out.has(String(id))) out.set(String(id), path);
      }
    }
  }
...
export function buildSearchUrls(nameEn) {
  ...
  return {
    sale: `https://www.ddproperty.com/en/property-for-sale?market=residential&freetext=${q}`,
    rent: `https://www.ddproperty.com/en/property-for-rent?market=residential&freetext=${q}`,
  };
}
...
export function listingDetailUrl(id, path = null) {
  if (path) {
    ...
    return `https://www.ddproperty.com${path.startsWith("/") ? path : `/${path}`}`;
  }
  return `https://www.ddproperty.com/en/listing/${id}`;
}
...
export function parseListingType(product, nextListing) {
  const scoped = [
    product?.name,
    product?.description,
    nextListing?.title,
    nextListing?.typeCode,
    nextListing?.listingType,
  ]
    .filter(Boolean)
    .join("\n");
  const price = Number(
  ...
  const hasSale = /for\s*sale|ขาย|sale/i.test(scoped);
  const hasRent = /for\s*rent|ให้เช่า|rent/i.test(scoped);
...
/**
 * Parse detail HTML into raw record. Returns null if challenge/unparseable.
 */
export function parseDetailHtml(html, listingId, pageUrl) {
  ...
  const product = ld.find((x) => x?.["@type"] === "Product" || x?.["@type"] === "Apartment" || x?.["@type"] === "Residence") || null;
  ...
  const currency =
    ...
    "THB";
  const listingType = parseListingType(product, nextListing);
  ...
  // Prefer structured beds/baths/area from NEXT when present
  if (nextListing?.bedrooms != null) specs.bedrooms = Number(nextListing.bedrooms);
  if (nextListing?.bathrooms != null) specs.bathrooms = Number(nextListing.bathrooms);
...
export function toListingDto(raw, ctx) {
  ...
  const base = {
    external_ref: `ddproperty-${raw.listing_id}`,
    ...
    summary: i18nTriple(
      `${raw.listing_type === "sale" ? "Sale" : "Rent"}: ${beds ?? "?"}BR, ${area ?? "?"} sq.m., ${price.toLocaleString("en-US")} THB. Source DDproperty #${raw.listing_id}.`,
      `${raw.listing_type === "sale" ? "出售" : "出租"}：${beds ?? "?"}卧，${area ?? "?"}㎡，${price.toLocaleString("en-US")} 泰铢。来源 DDproperty #${raw.listing_id}。`,
      `${raw.listing_type === "sale" ? "ขาย" : "เช่า"}: ${beds ?? "?"} นอน ${area ?? "?"} ตร.ม. ราคา ${price.toLocaleString("en-US")} บาท (DDproperty #${raw.listing_id})`,
    ),
    description: i18nTriple(
      `Normalized from DDproperty listing ${raw.listing_id}. Specs/price from public detail (ld+json / __NEXT_DATA__). Availability subject to source.`,
      `来自 DDproperty 挂牌 ${raw.listing_id}。规格与价格取自公开详情页；是否可售/可租以来源页为准。`,
      `แปลงจากประกาศ DDproperty ${raw.listing_id} ยึดข้อมูลจากหน้าต้นทาง`,
    ),
    featured: false,
    source: SOURCE,
    listing_url: `https://www.ddproperty.com/en/listing/${raw.listing_id}`,
