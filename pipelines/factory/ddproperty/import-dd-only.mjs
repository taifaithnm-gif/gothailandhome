#!/usr/bin/env node
/**
 * DDproperty-only importer.
 * - Reads listings-ddproperty.json only
 * - Resolves existing project by slug (no parent rewrites)
 * - Upserts ONLY rows with source=ddproperty
 * - Never updates PropertyHub or LivingInsider properties
 */
import {
  existsSync,
  mkdirSync,
...
const OUT = join(ROOT, "pipelines/factory/ddproperty/_runs");
...
// Additive source priority row (not a schema change)
await admin.from("listing_source_priority").upsert(
  {
    source: "ddproperty",
    priority: 22,
    notes: "DDproperty Wave 1 adapter",
  },
  { onConflict: "source" },
);
...
const projects = readdirSync(join(ROOT, "content/projects"))
  .filter((d) =>
    existsSync(join(ROOT, "content/projects", d, "listings-ddproperty.json")),
  )
  .filter((d) => {
    const n = (
      JSON.parse(
        readFileSync(
          join(ROOT, "content/projects", d, "listings-ddproperty.json"),
          "utf8",
        ),
      ).listings || []
    ).length;
...
if (!projects.length) {
  ...
  report.note =
    "No listings-ddproperty.json packages with listings — nothing to import (harvest empty/blocked).";
  writeFileSync(join(OUT, "import-dd-only.json"), JSON.stringify(report, null, 2));
...
for (const slug of projects) {
  const pkg = JSON.parse(
    readFileSync(
      join(ROOT, "content/projects", slug, "listings-ddproperty.json"),
      "utf8",
    ),
  );
  ...
  for (const listing of pkg.listings || []) {
    if (listing.source && listing.source !== "ddproperty") {
      report.skipped += 1;
      continue;
    }
    ...
    if (existing?.source && existing.source !== "ddproperty") {
      report.errors.push({
        slug,
        external_ref: listing.external_ref,
        error: "refused_non_ddproperty_row",
      });
      continue;
    }
    ...
    if (propertyId && (prev == null || prev !== next)) {
      await admin.from("listing_price_history").insert({
        ...
        listing_url: listing.listing_url,
        observed_at: now,
        note: prev == null ? "ddproperty_import" : "price_change",
      });
    ...
    await admin.from("listing_verification_events").insert({
      ...
      source: "ddproperty",
      source_listing_id: identity.source_listing_id,
      event_type: "ddproperty_import",
      to_status: verificationStatus,
      evidence: { external_ref: listing.external_ref, project_slug: slug },
      batch_key: `ddproperty-wave1-${now.slice(0, 10)}`,
    });
    report.events += 1;
  }
