...
import { renderInvestmentGuideLocale } from "@/lib/content";
...
export async function generateMetadata({
  ...
  const guide = renderInvestmentGuideLocale(lang);
  if (!guide) return {};
  return buildPageMetadata({
    locale: lang,
    title: guide.title.value,
    description: guide.summary.value,
    path: "/knowledge/investment",
  });
}
...
export default async function InvestmentGuidePage({
  ...
  return (
    ...
    >
      ...
      />
      <article className="space-y-6" data-slot="investment-guide">
        <SurfaceCard tone="soft" className="space-y-2 p-5!">
          <p className="text-xs font-medium text-[var(--brand)]">{g.educationalLabel}</p>
          ...
          <p className="text-xs text-stone-500">{guide.forecastDisclaimer.value}</p>
        ...
        <ContentRelatedLinks
          locale={lang}
          dict={dict}
          currentPath="/knowledge/investment"
        />
      </article>
    </PageShell>
  );
