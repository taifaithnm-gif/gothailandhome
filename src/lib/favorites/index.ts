/**
 * P1-15 — Accountless favorites state contract.
 *
 * Local-device only. No account, backend, sync, or CRM.
 * Persist only stable property identifiers (id and/or slug).
 */

/** Schema version written to storage. */
export const FAVORITES_SCHEMA_VERSION = 1;

/**
 * localStorage key for the current schema.
 * Older unknown keys are ignored (fail closed).
 */
export const FAVORITES_STORAGE_KEY = "gth.favorites.v1";

/** Maximum saved properties per device. Overflow drops oldest first. */
export const FAVORITES_MAX_ITEMS = 50;

/**
 * Retention / privacy wording for local-device favorites (G-PRODUCT-FAV).
 * Safe to surface in UI later; not a legal page.
 */
export const FAVORITES_RETENTION_NOTE =
  "Favorites are stored only on this device. They are not linked to an account, not synced across devices, and are removed if you clear site data for this browser.";

const MAX_ID_LENGTH = 80;
const MAX_SLUG_LENGTH = 120;
const SLUG_PATTERN = /^[a-z0-9]+(?:-[a-z0-9]+)*$/i;
const ID_PATTERN = /^[a-zA-Z0-9_-]+$/;

export type FavoriteRef = {
  /** Stable DB / row id when known. */
  id?: string;
  /** Stable public slug used by `/properties/[id]` routes. */
  slug?: string;
};

export type FavoritesState = {
  version: number;
  items: FavoriteRef[];
};

export type FavoritesStorage = {
  getItem(key: string): string | null;
  setItem(key: string, value: string): void;
  removeItem(key: string): void;
};

export function emptyFavoritesState(): FavoritesState {
  return { version: FAVORITES_SCHEMA_VERSION, items: [] };
}

function normalizeId(raw: unknown): string | undefined {
  if (typeof raw !== "string") return undefined;
  const value = raw.trim();
  if (!value || value.length > MAX_ID_LENGTH) return undefined;
  if (!ID_PATTERN.test(value)) return undefined;
  return value;
}

function normalizeSlug(raw: unknown): string | undefined {
  if (typeof raw !== "string") return undefined;
  const value = raw.trim().toLowerCase();
  if (!value || value.length > MAX_SLUG_LENGTH) return undefined;
  if (!SLUG_PATTERN.test(value)) return undefined;
  return value;
}

/**
 * Normalize a candidate favorite ref.
 * Requires at least one stable identifier; drops all other fields.
 */
export function normalizeFavoriteRef(
  raw: unknown,
): FavoriteRef | null {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
  const obj = raw as Record<string, unknown>;
  const id = normalizeId(obj.id);
  const slug = normalizeSlug(obj.slug);
  if (!id && !slug) return null;
  const out: FavoriteRef = {};
  if (id) out.id = id;
  if (slug) out.slug = slug;
  return out;
}

function refsEqual(a: FavoriteRef, b: FavoriteRef): boolean {
  if (a.id && b.id && a.id === b.id) return true;
  if (a.slug && b.slug && a.slug === b.slug) return true;
  return false;
}

function findIndex(items: FavoriteRef[], ref: FavoriteRef): number {
  return items.findIndex((item) => refsEqual(item, ref));
}

/**
 * Parse stored JSON. Corrupt, empty, or unsupported versions fail closed
 * to an empty v1 state (never throw).
 */
export function parseFavoritesState(
  raw: string | null | undefined,
): FavoritesState {
  if (raw == null || raw === "") return emptyFavoritesState();

  let parsed: unknown;
  try {
    parsed = JSON.parse(raw);
  } catch {
    return emptyFavoritesState();
  }

  if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
    return emptyFavoritesState();
  }

  const obj = parsed as Record<string, unknown>;
  const version =
    typeof obj.version === "number" && Number.isInteger(obj.version)
      ? obj.version
      : null;

  // Unknown / future versions fail closed rather than guessing.
  if (version == null || version < 1 || version > FAVORITES_SCHEMA_VERSION) {
    return emptyFavoritesState();
  }

  const rawItems = Array.isArray(obj.items) ? obj.items : [];
  const items: FavoriteRef[] = [];
  const seenIds = new Set<string>();
  const seenSlugs = new Set<string>();

  for (const entry of rawItems) {
    const ref = normalizeFavoriteRef(entry);
    if (!ref) continue;
    if (ref.id && seenIds.has(ref.id)) continue;
    if (ref.slug && seenSlugs.has(ref.slug)) continue;
    if (ref.id) seenIds.add(ref.id);
    if (ref.slug) seenSlugs.add(ref.slug);
    items.push(ref);
    if (items.length >= FAVORITES_MAX_ITEMS) break;
  }

  return { version: FAVORITES_SCHEMA_VERSION, items };
}

/** Serialize for storage. Strips anything outside the contract. */
export function serializeFavoritesState(state: FavoritesState): string {
  const normalized = parseFavoritesState(
    JSON.stringify({
      version: FAVORITES_SCHEMA_VERSION,
      items: state.items,
    }),
  );
  return JSON.stringify({
    version: normalized.version,
    items: normalized.items.map((item) => {
      const out: FavoriteRef = {};
      if (item.id) out.id = item.id;
      if (item.slug) out.slug = item.slug;
      return out;
    }),
  });
}

export function hasFavorite(
  state: FavoritesState,
  ref: FavoriteRef,
): boolean {
  const normalized = normalizeFavoriteRef(ref);
  if (!normalized) return false;
  return findIndex(state.items, normalized) >= 0;
}

/**
 * Add a favorite. Deterministic:
 * - invalid ref → unchanged
 * - duplicate → move to end (most recent)
 * - over max → drop oldest until within bound
 */
export function addFavorite(
  state: FavoritesState,
  ref: FavoriteRef,
): FavoritesState {
  const normalized = normalizeFavoriteRef(ref);
  if (!normalized) {
    return {
      version: FAVORITES_SCHEMA_VERSION,
      items: [...state.items],
    };
  }

  const items = state.items.filter((item) => !refsEqual(item, normalized));
  items.push(normalized);
  while (items.length > FAVORITES_MAX_ITEMS) {
    items.shift();
  }

  return { version: FAVORITES_SCHEMA_VERSION, items };
}

export function removeFavorite(
  state: FavoritesState,
  ref: FavoriteRef,
): FavoritesState {
  const normalized = normalizeFavoriteRef(ref);
  if (!normalized) {
    return {
      version: FAVORITES_SCHEMA_VERSION,
      items: [...state.items],
    };
  }

  return {
    version: FAVORITES_SCHEMA_VERSION,
    items: state.items.filter((item) => !refsEqual(item, normalized)),
  };
}

export function clearFavorites(): FavoritesState {
  return emptyFavoritesState();
}

export function listFavoriteSlugs(state: FavoritesState): string[] {
  return state.items
    .map((item) => item.slug)
    .filter((slug): slug is string => Boolean(slug));
}

export function listFavoriteIds(state: FavoritesState): string[] {
  return state.items
    .map((item) => item.id)
    .filter((id): id is string => Boolean(id));
}

/** In-memory adapter for tests and SSR-safe defaults. */
export function createMemoryFavoritesStorage(
  initial?: Record<string, string>,
): FavoritesStorage {
  const map = new Map<string, string>(Object.entries(initial ?? {}));
  return {
    getItem(key) {
      return map.has(key) ? map.get(key)! : null;
    },
    setItem(key, value) {
      map.set(key, String(value));
    },
    removeItem(key) {
      map.delete(key);
    },
  };
}

/**
 * Browser localStorage adapter. Missing/blocked storage fails closed
 * (get → null; set/remove no-op) without throwing.
 */
export function createBrowserFavoritesStorage(
  storage?: Pick<Storage, "getItem" | "setItem" | "removeItem"> | null,
): FavoritesStorage {
  const store =
    storage ??
    (typeof globalThis !== "undefined" &&
    "localStorage" in globalThis &&
    globalThis.localStorage
      ? globalThis.localStorage
      : null);

  return {
    getItem(key) {
      if (!store) return null;
      try {
        return store.getItem(key);
      } catch {
        return null;
      }
    },
    setItem(key, value) {
      if (!store) return;
      try {
        store.setItem(key, value);
      } catch {
        // Quota / privacy mode — fail closed.
      }
    },
    removeItem(key) {
      if (!store) return;
      try {
        store.removeItem(key);
      } catch {
        // ignore
      }
    },
  };
}

export function loadFavorites(
  storage: FavoritesStorage,
  key: string = FAVORITES_STORAGE_KEY,
): FavoritesState {
  return parseFavoritesState(storage.getItem(key));
}

export function saveFavorites(
  storage: FavoritesStorage,
  state: FavoritesState,
  key: string = FAVORITES_STORAGE_KEY,
): FavoritesState {
  const normalized = parseFavoritesState(serializeFavoritesState(state));
  storage.setItem(key, serializeFavoritesState(normalized));
  return normalized;
}

export function clearFavoritesStorage(
  storage: FavoritesStorage,
  key: string = FAVORITES_STORAGE_KEY,
): FavoritesState {
  storage.removeItem(key);
  return emptyFavoritesState();
}
